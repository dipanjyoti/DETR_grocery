...
def main(args):
    ...
    if args.finetune:
        ...
        state_dict=load_model(args, state_dict)

        missing_keys, unexpected_keys = model_without_ddp.load_state_dict(state_dict, strict=False)
        
        for key in missing_keys: 
            tensor = model_without_ddp.state_dict()[key]
            # size = tensor.size()
            # if len(size) < 2:
            #     init.uniform_(tensor, a=-0.5, b=0.5)
            # else:
            #     init.xavier_uniform_(tensor, gain=init.calculate_gain('relu'))
